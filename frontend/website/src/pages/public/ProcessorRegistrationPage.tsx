...
export const ProcessorRegistrationPage: React.FC = () => {
  ...
  const validateStep3 = (): boolean => {
    ...
    if (!step3Data.company_name.trim()) {
      newErrors.company_name = 'Company name is required';
    }
    if (!step3Data.city.trim()) {
      newErrors.city = 'City is required';
    }
    if (!/^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$/.test(step3Data.gstin)) {
      newErrors.gstin = 'Invalid GSTIN format';
    }
    if (!/^\d{14}$/.test(step3Data.fssai_license)) {
      newErrors.fssai_license = 'FSSAI license must be 14 digits';
    }
  ...
  const handleStep1Submit = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await processorService.registerStep1(step1Data);
      if (response.error) {
  ...
  const handleStep2Submit = async () => {
    ...
    try {
      ...
      const response = await processorService.registerStep2(dataWithToken);
      if (response.error) {
        alert(response.error);
      } else if (response.data) {
  ...
  const handleStep3Submit = async () => {
    ...
    try {
      ...
      const response = await processorService.registerStep3(dataWithToken);
      if (response.error) {
        alert(response.error);
      } else if (response.data) {
        ...
        alert('Processor registered successfully! Awaiting verification.');
        navigate('/dashboard');
      }
    } catch (error) {
          ...
          {currentStep === 3 && (
            <div className="space-y-6">
              ...
              <Select
                ...
                name="business_scale"
                value={step3Data.business_scale}
                onChange={handleStep3Change}
                required
                options={[
