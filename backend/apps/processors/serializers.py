...
class ProcessorRegistrationStep2Serializer(serializers.Serializer):
    ...
    # Address (UserProfile fields only - NO city!)
...
class ProcessorRegistrationStep3Serializer(serializers.Serializer):
    """Step 3: Processor Business Details"""
    # Business Info
    company_name = serializers.CharField(max_length=200)
    business_scale = serializers.ChoiceField(
        choices=Processor.BUSINESS_SCALE_CHOICES
    )
    
    # Location (Processor-specific - city goes here!)
    ...
    fssai_license = serializers.CharField(max_length=14)
    ...
    def validate_gstin(self, value):
        ...
        if Processor.objects.filter(gstin=value).exists():
            raise serializers.ValidationError("GSTIN already registered")
        return value
    
    def validate_fssai_license(self, value):
        if not re.match(r'^\d{14}$', value):
            raise serializers.ValidationError("FSSAI license must be 14 digits")
        if Processor.objects.filter(fssai_license=value).exists():
            raise serializers.ValidationError("FSSAI license already registered")
        return value
    ...
    def validate_monthly_capacity(self, value):
        if value <= 0:
    ...
    def validate_monthly_requirement(self, value):
        if value <= 0:
            raise serializers.ValidationError("Requirement must be greater than 0")
        return value


class ProcessorRegistrationCompleteSerializer(serializers.Serializer):
    """Complete Processor Registration Data"""
    # Step 1
    ...
    # Step 2
    profile = ProcessorRegistrationStep2Serializer()
    
    # Step 3
    processor = ProcessorRegistrationStep3Serializer()
...
class ProcessorSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Processor
        fields = [
            'id', 'user', 'user_details', 'company_name', 'processor_code',
            'business_scale', 'city', 'state', 'pincode',
            'gstin', 'fssai_license',
            'monthly_capacity', 'monthly_requirement',
            'contact_person', 'contact_phone', 'contact_email',
            'is_verified', 'is_active', 'created_at', 'updated_at'
        ]
        read_only_fields = ['id', 'processor_code', 'created_at', 'updated_at', 'user_details']
...
class ProcessorListSerializer(serializers.ModelSerializer):
    """Lightweight processor list"""
    user_name = serializers.CharField(source='user.full_name', read_only=True)
    
    class Meta:
        model = Processor
        fields = [
            'id', 'processor_code', 'company_name', 'user_name',
            'city', 'state', 'business_scale',
